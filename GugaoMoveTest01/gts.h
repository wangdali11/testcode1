...
typedef struct
{
	...
	short accStartPercent; //加速段起始加速度百分比
	short decEndPercent; //减速段终点加速度百分比
} TMoveAbsolutePrmEx;
...
GT_API GT_BufWaitDi(short crd, short diType, unsigned short diIndex,unsigned short level, 
	short continueTime, long overTime, short flagMode,long segNum=0, short fifo=0);
GT_API GT_BufWaitLongVar(short crd, short index, long value, long overTime, short flagMode, long segNum, short fifo);
GT_API GT_BufSetLongVar(short crd,short index,long value,short fifo);
...
typedef struct
{
	...
	long io_length;
	unsigned long Vid;
	unsigned long Pid;	int io_type;             //1 输入  2 输出  3 输入输出
	int dump[10];
} TSlaveInfo;
...
{
	short dcError; 
	short workingCountErrorCnt;
	short ecatCommStatus;
	short workingCount;
	short offlineFlag;
	short dump;
	short dump1;
...
GT_API GT_SetEcatHomingPrmEx(short axis,short *psMethod,double *pdSpeed1,double *pdSpeed2,double *pdAcc,long *plOffset,unsigned short *pusProbeFunction);
...
GT_API GT_GetEcatDcErrorEx(TEcatErrInfo *pEcatErrorInfo);
...
GT_API GT_RelateEcatSlaveToMcAuEncoder(short auenc,short ecatAxisIndex);
GT_API GT_RelateEcatSlaveToMcMpgEncoder(short mpg,short ecatAxisIndex);
...
GT_API GT_RelateEcatSlaveToMcMpgDiEx(short mpg,short ecatIndex,short ecatType,short bitoffset,short pdoOffset);
...
GT_API GTN_BufWaitDiEx(short core,short crd, short diType, unsigned short diIndex,unsigned short level, 
	short continueTime, long overTime, short flagMode,long segNum, short fifo);
...
GT_API GTN_GetBufWaitDiStatus(short core,short crd,short *pDiType,unsigned short *pDiIndex,unsigned short *pLevel, 
	short *pContinueTime,long *pOverTime,short *pFlagMode,long *pSegNum,short *pEnable,long *pCount,short fifo);
...
GT_API GT_StartAxisPressCompensateFixFactor(short axis,TAxisPressCompensateFixFactor *pPressComp);
GT_API GT_GetAxisPressCompensateFixFactorStatus(short axis,short *pFixFactorSts,TAxisPressCompensateFixFactor *pPressComp);
...
GT_API GT_StartAxisPressCompensateFixPid(short axis,TAxisPressCompensateFixPid *pPressComp);
GT_API GT_GetAxisPressCompensateFixPidStatus(short axis,short *pFixPidSts,TAxisPressCompensateFixPid *pPressComp);
